...
public class CreateExcelFile
{
    ...
    private static void AppendNumericCell(string cellReference, string cellStringValue, ref OpenXmlWriter writer)
    {
        ...
        string i = cellReference.Substring(1);
        CellFormula formula = new CellFormula();
        if (cellReference[0] == 'G')
        {
            formula = new CellFormula("=17.18*E"+i);
            //formula.CalculateCell = true;
        }
        if (cellReference[0] == 'H')
        {
            formula = new CellFormula("=1193.96*F" + i);
           // formula.CalculateCell = true;
        }
        if (cellReference[0] == 'I')
        {
            formula = new CellFormula("=G" + i+"+H"+i);
           // formula.CalculateCell = true;
        }
        if (cellReference[0] == 'l')
        {
            formula = new CellFormula("=ЕСЛИ(B"+i+"<5;5;3)");
            //formula.CalculateCell = true;
        }
       /* if (cellReference[0] == 'J')
        {
            formula = new CellFormula("=ЕСЛИ(AB"+i+"=1;H"+i+";0)");
            //formula.CalculateCell = true;
        }
        * */
        //  Add a new Excel Cell to our Row 
